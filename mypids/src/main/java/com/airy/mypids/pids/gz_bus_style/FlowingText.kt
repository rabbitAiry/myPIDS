...
import com.airy.mypids.pids.gz_bus_style.LineMapLinearLayout.Status
import com.airy.mypids.utils.ResUtil

private const val TAG = "FlowingText"
class FlowingText @JvmOverloads constructor(
    ...
    private var statusAnimation = ValueAnimator.ofFloat()
    ...
    private fun drawText(x: Float, y: Float, canvas: Canvas, height: Float): Float {
        ...
        for ((idx, c) in text.withIndex()) {
            canvas.drawText(c.toString(), width / 8f, cy, textPaint)
    ...
    override fun onVisibilityChanged(changedView: View, visibility: Int) {
        ...
        if(visibility == View.GONE)handler.removeCallbacksAndMessages(null)
    }
}